...
def scrape_and_display(url):
    try:
        ...
        choice = easygui.buttonbox("Choose an option:", "Article Options", choices=["Read Summary (Beta)", "Read Original"])
        if choice == "Read Summary (Beta)":
            summary = generate_summary(response.text, summary_length=100)
            print("Summary:")
            print(summary)
...
site_urls = ["https://nascecresceignora.it/", "https://example.org"]
...
if selected_site == "Enter custom URL...":
    site_url = easygui.enterbox("Enter the custom website URL:")
    if not site_url:
